...
- Treat `docs/.vitepress/config.mts` as the canonical VitePress configuration and `package.json` as the source of documentation commands and dependencies.
...
1. Install dependencies with `npm install --no-audit --no-fund` when `node_modules/` is absent or dependencies changed.
